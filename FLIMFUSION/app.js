const API_KEY = "3573bb2eb6904bafcc0209feb032451d";
...
async function showPopUp(card) {
  ...
  heart_icon.addEventListener("click", () => {
    if (heart_icon.classList.contains("change-color")) {
      ...
      localStorage.setItem(`${movie.id}`, data);
    }
  });
}
function addFavouriteMoviesToDOM() {
  for (let i = 0; i !== null; i++) {
    const key = localStorage.key(i);
    const name = JSON.parse(localStorage.getItem(key));
    mainGrid.innerHTML += `<div class="card"  data-id="${name.id}">
  <div class="img">
    <img src=${imagePath + name.poster_path} alt="" />
  </div>
  <div class="info">
    <h2>${name.title}</h2>
    <div class="single-info">
      <span>Rating :</span>
      <span>${name.vote_average}/10</span>
    </div>
    <div class="single-info">
      <span>Release Date : </span>
